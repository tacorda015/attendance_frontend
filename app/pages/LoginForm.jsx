      ...
      <div className="flex flex-col gap-3 items-center border shadow rounded-md min-h-96 h-full w-96 bg-white p-4 text-center">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 w-full">
          <label className="form-control w-full">
            ...
            <input type="text" name="username" placeholder="Username" className="input input-bordered w-full" autoComplete={'off'} required />
          </label>
          ...
          <button type="submit" className="btn btn-primary text-white disabled:bg-primary disabled:opacity-55 disabled:text-slate-200" disabled={loading}>
            {loading ? (
              <>
                Logging in <span className="loading loading-spinner text-white"></span>
              </> ) : ( "Login" )}
          </button>
